...
This is a finite state transducer from another Wikipedia example. It recognizes a regular relation A = {0, 1}, B = {0, 1} such that for state *q* and input *a*, output is ω(s<sub>i</sub>, a: a &x2282; A) = 0, ω(s<sub>0</sub>, 1) = 1, ω(s<sub>1</sub>, 0) = 1, and ω(a: q &x2282; {s<sub>0</sub>, s<sub>1</sub>}, a: a &x2282; A) = 0. Simply put, it's an edge detector that outputs 1 if the input changes and 0 if it stays the same.
